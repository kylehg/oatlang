...
type uid = int * string                 (* Local identifiers *)
type gid = string                       (* Global identifiers *)
type lbl = X86.lbl                      (* Labels *)

let string_of_uid (i,s) = Printf.sprintf "%s%d" s i
let string_of_gid g = g

(* [mk_uid] should be the only way to generate a new id, which guarantees its
   unique postfix *)
let mk_uid : string -> uid =
  let ctr = ref 0  in
  fun h -> incr ctr; (!ctr, h)

let mk_gid : string -> gid =
  let ctr = ref 0 in
  fun h -> incr ctr; Printf.sprintf "%s%d" h !ctr

(* Make a global id with the given name. This should only be used for compiling
   external function declarations. *)
let mk_gid_unsafe (str:string) : gid = str
